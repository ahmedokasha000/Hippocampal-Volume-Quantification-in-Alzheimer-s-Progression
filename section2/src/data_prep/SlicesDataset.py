...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        vol_id,slice_n=slc
        sample["image"]=torch.from_numpy((self.data[vol_id]["image"])[slice_n]).type(torch.cuda.FloatTensor)
        sample["seg"]=torch.from_numpy((self.data[vol_id]["seg"])[slice_n]).type(torch.cuda.LongTensor)
