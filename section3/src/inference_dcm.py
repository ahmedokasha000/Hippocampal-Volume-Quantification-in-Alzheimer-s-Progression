...
def load_dicom_volume_as_numpy_from_list(dcmlist):
    ...
    slices = [np.flip(dcm.pixel_array,axis=0).T for dcm in sorted(dcmlist, key=lambda dcm: dcm.InstanceNumber)]
...
def get_predicted_volumes(pred):
    ...
    ## this is done because the output of the inference was left to be
    ## with dimension of [axial_slices,channels,patch_size,patch_size]
    #print ("prediction shape= ",pred.shape)
    ant_ch=(pred[:,1,:,:]>0.5).astype(float) ## anterior channel
    post_ch=(pred[:,2,:,:]>0.5).astype(float)## posterior channel
    volume_ant= ant_ch.sum()
    volume_post= post_ch.sum()
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 150),
                        f"Patient ID: {header.PatientID} Hippocampus Volume: {inference['total']} anterior vol:{inference['anterior']} posterior vol:{inference['posterior']}",
...
def get_series_for_inference(path):
    ...
    # It is important to note that radio logical modalities most often operate in terms
    ...
    for objectf in dicoms:
        #print(objectf.SeriesDescription)
        if (objectf.SeriesDescription == "HippoCrop"):
            series_for_inference.append(objectf)
            
    print("series length",len(series_for_inference))
...
if __name__ == "__main__":
    ...
    ##this function calls the inference inside of it 
    ...
    os_command("storescu 127.0.0.1 4242 -v -aec HIPPOAI /home/workspace/out/report.dcm")
